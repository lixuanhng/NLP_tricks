...
src_file = config.FLAGS.src_file  # 分词后的文本数据
tgt_file = config.FLAGS.tgt_file  # 实体标注后的文本数据

# 只有在预测结果时使用。训练完成后这些就都有了
pred_file = config.FLAGS.pred_file  # 需要预测的数据文件
src_vocab_file = config.FLAGS.src_vocab_file  # 训练文本的词表
tgt_vocab_file = config.FLAGS.tgt_vocab_file  # 训练文本的标注词表
word_embedding_file = config.FLAGS.word_embedding_file  # 词向量文件
model_path = config.FLAGS.model_path  # 模型路径
embeddings_size = config.FLAGS.embeddings_size  # 词向量维度
max_sequence = config.FLAGS.max_sequence  # 最大文本长度
...
def build_word_index():
    ...
    if not os.path.exists(word_embedding_file):
        ...
        print('不存在词向量文件，请检查文件路径')
        return

    # print('building word index...')
    print('创建词索引')
    if not os.path.exists(src_vocab_file):
        with open(src_vocab_file, 'w') as file1:
...
def get_predict_iterator(src_vocab_table, vocab_size, batch_size, max_len=max_sequence):
    ...
    pred_dataset = pred_dataset.map(lambda src: tf.string_split([src]).values)
    if max_len:
        pred_dataset = pred_dataset.map(lambda src: src[:max_sequence])

    pred_dataset = pred_dataset.map(lambda src: tf.cast(src_vocab_table.lookup(src), tf.int32))
