"""
这部分的代码是用 keras实现的 Attention
代码链接：https://github.com/bojone/attention
博客链接：https://kexue.fm/archives/4765
"""
...
def to_mask(x, mask, mode='mul'):
    """通用 mask函数
    这里的 mask.shape=[batch_size, seq_len]或[batch_size, seq_len, 1]
    """
    if mask is None:
        return x
    else:
